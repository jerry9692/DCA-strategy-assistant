...
- #24 历史场景：预设 2020 熔断、2022 加息、2023 修复等历史区间，一键切换回测区间。
...
- #33 滚动窗口表现：回测结果新增滚动 3 年 / 1 年资金年化收益图，用于观察策略是否只在少数年份表现好。
...
**完成情况**：后端在 `/api/backtests/run` 返回 `rollingPerformance`，根据区间长度自动选择滚动 3 年或 1 年；前端新增滚动表现图，同时展示本策略、固定 DCA 和一次性买入。收益口径采用窗口内新增投入的 IRR，避免时间加权收益把不同投入节奏画成同一条线。
