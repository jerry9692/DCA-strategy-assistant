...
- #33 滚动窗口表现：回测结果新增滚动 3 年 / 1 年现金流调整后年化收益图，用于观察策略是否只在少数年份表现好。
...
**完成情况**：后端在 `/api/backtests/run` 返回 `rollingPerformance`，根据区间长度自动选择滚动 3 年或 1 年；前端新增滚动表现图，同时展示本策略、固定 DCA 和一次性买入。收益口径采用现金流调整后的时间加权年化，避免新增定投资金扭曲滚动收益。
