...
> **进度（2026-05-23 更新）**：P0（A1-A6）+ P1（B1-B4）+ P2 第一档（C2/C4/C5/C6）+ C1/C3 已合到 main。
> 
...
> 下一档主线建议是 D1（滚动窗口表现）或 C7/C8（自动调优体验收尾）。C1/C3 已完成，
> 后续 D1/D2 等扩展功能可以继续沿用同一套 URL state 机制。
...
- **完成情况**：C1 第一版已完成。美股新增核心宽基、红利价值、国际股票、债券防守、商品替代和高级/高波动 ETF，共 26 个内置美股 ETF；A 股新增上证 50、沪深 300、中证 500、创业板、科创 50 五个基础指数 ETF。前端按分组展示，高波动/杠杆/比特币 ETF 显示风险提示；A 股通过 `providerSymbol` 映射到 Yahoo Finance 的 `.SS/.SZ` 数据源代码，金额展示切换为人民币符号。
...
1. **D1 滚动窗口表现**（1 天）—— 差异化、低成本、高价值。从 P3 跳上来。
2. **C7/C8 自动调优体验收尾**（半天）—— 减少长任务误取消，保留已应用结果。
3. **C9-C10**（暗色细节、CSV 体验等纯打磨）—— 按精力随时挑。
