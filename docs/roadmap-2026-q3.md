...
> **进度（2026-05-29 更新）**：P0（A1-A6）+ P1（B1-B4）+ P2（C1-C10）+ P3 D1 + E2 解读部分已合到 main。
> 
> - P2 已完成：C2 费率滑点、C4 指标 hover、C5 日期范围、C6 错误重试。
> - 顺手修复：长回测下 priceSeries 子采样导致买入点偏离价格线（详见
>   change-log）。
> - E2 LLM 解读已完成（智能问答部分待做）：接入 OpenAI 兼容 API，用自然语言
>   解释本期建议金额。详见 [`change-log/2026-05-29-e2-llm-explanation.md`](./change-log/2026-05-29-e2-llm-explanation.md)。
> 
> 详见 [`change-log/2026-05-22-p2-usability-pass.md`](./change-log/2026-05-22-p2-usability-pass.md)。
> 下一档主线建议是 D3（蒙特卡洛模拟）或 D2（多标的组合定投），以及 E2 后半的智能问答。
...
- **完成情况**：已在回测结果中返回 `rollingPerformance`。长区间自动使用滚动 3 年资金年化，2-5 年区间使用滚动 1 年资金年化，短于约 2 年时不展示。口径采用窗口内 IRR：假设从每个滚动窗口起点才开始执行对应方案，只把窗口内新增投入和窗口末市值作为现金流，前端同时展示本策略、固定 DCA 和一次性买入，用来观察表现是否集中在少数年份。
...
- "为什么本周建议投 120 美元而不是 100？"用 LLM 综合 marketState + signals + reasons 生成自然语言解读；后续支持自由提问。
- **解读部分已完成**：后端新增 `explanations.py` 模块和 `/api/explanations/run` 端点，通过 httpx 转发用户 API Key 到 OpenAI 兼容服务。前端右侧参数面板底部有 LLM 设置卡片，填入 API Key 后建议卡下方自动显示白话解读。Key 仅存浏览器 localStorage，不落盘不记录。支持 OpenAI / DeepSeek / Moonshot / 智谱等兼容服务商，只需改 baseUrl 即可。
- **智能问答待做**：用户可以在解读下方自由输入问题（如"如果下周跌 5%，策略会怎么建议？"），LLM 结合当前 marketState + signals + strategy 上下文回答。与单次解读不同，问答需要维护对话历史和 context window。
- 价值：降低非专业用户门槛。
...
这次代码审查的三轮工作（P0 5 项 + P1 3 项 + P2 6 项）已经把"代码层面的债务"清干净了。从这里开始，**工程基础（CI、测试、Docker）和架构债（拆 main.tsx、优化器并行）应该是下个月的主线**。功能层面，**滚动窗口 + LLM 解读 + 多标的 + 蒙特卡洛**是最能拉开差距的四个方向，其中滚动窗口和 LLM 解读已于 Q2 完成。
