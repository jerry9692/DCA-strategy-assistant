...
> **进度（2026-05-22 更新）**：P0（A1-A6）+ P1（B1-B4）+ P2 第一档（C2/C4/C5/C6）已合到 main。
> 
...
> 下一档主线建议是 C3（URL state 同步）—— P2 里第一个带增长机制的条目，
> 也是后续 D1/D2 等扩展功能的 state 同步前置。
...
1. **C3 URL state 同步**（半天）—— P2 里第一个带增长机制的条目。
   做完后用户分享配置 = 免费传播；后续 D1/D2 扩展 state 也会受益。
2. **C1 支持更多标的**（1-2 天）—— 用户面价值最高。注意会暴露被
   3 标的掩盖的边角问题（高波动 ETF 在 RSI/grid 下的表现、债券 ETF
   回撤太小让 drawdown_boost 几乎不触发）。
3. **D1 滚动窗口表现**（1 天）—— 差异化、低成本、高价值。从 P3 跳上来。
4. **C7-C10**（暗色细节、CSV、cancel 触发收窄等）—— 按精力随时挑。
